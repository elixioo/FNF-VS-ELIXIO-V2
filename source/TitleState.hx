...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		if(!FlxG.save.data.psykaEasterEgg || !easterEggEnabled) {
			gfDance = new FlxSprite(FlxG.width * 0.5, FlxG.height * 0.3);
		...
		// FlxTween.tween(logoBl, {y: logoBl.y + 20}, 0.1, {ease: FlxEase.quadInOut, type: PINGPONG});
		// FlxTween.tween(logo, {y: logoBl.y + 20}, 0.1, {ease: FlxEase.quadInOut, type: PINGPONG, startDelay: 0.1});
		...
		logoSpr = new FlxSprite(0, FlxG.height * 0.2).loadGraphic(Paths.image('titlelogo'));
		add(logoSpr);
		logoSpr.visible = false;
		logoSpr.setGraphicSize(Std.int(logoSpr.width * 0.33));
		logoSpr.updateHitbox();
		logoSpr.screenCenter(X);
		logoSpr.antialiasing = ClientPrefs.globalAntialiasing;

		FlxTween.tween(credTextShit, {y: credTextShit.y + 1}, 2.9, {ease: FlxEase.quadInOut, type: PINGPONG});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				case 3:
					addMoreText('Elixio', 45);
					addMoreText('n0mdomestudios', 45);
				// credTextShit.text += '\npresent...';
				...
				// credTextShit.text = 'lov u newgrounds <3';
				// credTextShit.screenCenter();
				case 9:
